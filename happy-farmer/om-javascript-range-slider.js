...
function OmRangeSlider(inputElement, inputValueStyle) {
    ...
    function initVisuals() {
        ...
        visualSliderRangeIndicator.classList.add(
        ...
        visualSliderButtonEnd.style.left =
            visualSliderRange.getBoundingClientRect().width -
            visualSliderButtonEnd.getBoundingClientRect().width +
            'px'
        ...
        const rangeStart =
            range[0] && range[0] >= settings.min && range[0] <= settings.max
                ? range[0]
                : settings.min
        const rangeEnd =
            range[1] && range[1] >= settings.min && range[1] <= settings.max
                ? range[1]
                : settings.max
    ...
    function refreshRangeIndicator() {
        ...
        const buttonStartMiddle = Math.round(
            parseInt(buttonStart.style.left) + buttonStartWidth / 2
        )
        const buttonEndMiddle = Math.round(
            elementRangeWidth -
                (parseInt(buttonEnd.style.left) + buttonEndWidth / 2)
        )
    ...
    function updateValues() {
        const rangePx =
            elementRange.offsetWidth -
            buttonStart.offsetWidth -
            buttonEnd.offsetWidth -
            1
        const startPx = parseInt(buttonStart.style.left)
        const endPx = parseInt(buttonEnd.style.left) - buttonEnd.offsetWidth

        const isFloat =
            parseFloat(settings.min) % 1 !== 0 ||
            parseFloat(settings.max) % 1 !== 0

        const diffRange = settings.max - settings.min
        let resultStart =
            (diffRange / rangePx) * startPx + parseFloat(settings.min)
    ...
    function getValidX(button, positionX) {
        ...
        if (button === buttonStart) {
            if (newX <= 0) {
                ...
                newX + buttonStart.getBoundingClientRect().width >=
                parseInt(buttonEnd.style.left)
            ) {
                newX =
                    parseInt(buttonEnd.style.left) -
                    buttonStart.getBoundingClientRect().width
            }
        } else if (button === buttonEnd) {
            if (
                newX >=
                elementRange.getBoundingClientRect().width -
                    buttonEnd.getBoundingClientRect().width
            ) {
                newX =
                    elementRange.getBoundingClientRect().width -
                    buttonEnd.getBoundingClientRect().width
                ...
                newX <=
                buttonStart.getBoundingClientRect().width +
                    parseInt(buttonStart.style.left)
            ) {
                newX =
                    buttonStart.getBoundingClientRect().width +
                    parseInt(buttonStart.style.left)
...
function SliderEventHelper(buttonStart, buttonEnd) {
    ...
    function handleButtonGrabbed(e) {
        ...
        const buttonStartPosStart =
            buttonStart.getBoundingClientRect()?.left ?? undefined
        const buttonEndPosStart =
            buttonEnd.getBoundingClientRect()?.left ?? undefined
        let buttonStartPosEnd = undefined
        let halfDistanceBetweenButtons = undefined
        if (buttonStartPosStart && buttonEndPosStart) {
            buttonStartPosEnd =
                buttonStartPosStart + buttonStart.getBoundingClientRect().width
            halfDistanceBetweenButtons =
                (buttonEndPosStart - buttonStartPosEnd) / 2
        }

        if (
            halfDistanceBetweenButtons !== undefined &&
            e.currentTarget === buttonEnd
        ) {
            if (
                getXOfMoveEvent(e) <
    ...
    function handleKeyDown(e) {
        ...
        if (e.keyCode === arrowLeft) {
            newX = that.validateNewPosition(
                activeButton,
                activeButton.offsetLeft - 1
            )
    ...
    function init() {
        buttonStart.addEventListener('mousedown', handleButtonGrabbed, {
            passive: false,
        })
        buttonStart.addEventListener('touchstart', handleButtonGrabbed, {
            passive: false,
        })
        buttonStart.addEventListener('touchend', handleButtonRelease, {
            passive: false,
        })
        buttonStart.addEventListener('touchcancel', handleButtonRelease, {
            passive: false,
        })
        buttonEnd.addEventListener('mousedown', handleButtonGrabbed, {
            passive: false,
        })
        buttonEnd.addEventListener('touchstart', handleButtonGrabbed, {
            passive: false,
        })
        buttonEnd.addEventListener('touchend', handleButtonRelease, {
            passive: false,
        })
        buttonEnd.addEventListener('touchcancel', handleButtonRelease, {
            passive: false,
        })
        ...
        window.addEventListener('touchend', handleButtonRelease, {
            passive: false,
        })
        window.addEventListener('touchcancel', handleButtonRelease, {
            passive: false,
        })
        ...
        window.addEventListener('touchmove', handleButtonMove, {
            passive: false,
        })
        window.addEventListener(
            'mouseout',
